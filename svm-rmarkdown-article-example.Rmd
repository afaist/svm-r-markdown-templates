...
title: "Шаблон и руководство по началу работы с вёрсткой статей в Markdown и Pandoc"
...
- name: Алексей Кнорре
  affiliation: Европейский Университет в Санкт-Петербурге
abstract: "Этот текст является введением в R Markdown, рассказывает про его преимущества, и показывает, как студентам, учёным и преподавателям делать статьи в R Markdown. Автором является Стивен Миллер, который написал этот текст по-английски с примерами базового синтаксиса R Markdown и дал минимальный рабочий шаблон того, как можно проводить анализ данных в R с помощью пакета `knitr`. Алексей Кнорре изменил шаблон (теперь он собирается в PDF даже с русским языком) и немного перевёл начало."
